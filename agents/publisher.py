...
def _wp_auth_headers():
    user = os.getenv("WORDPRESS_USERNAME"); pwd = os.getenv("WORDPRESS_PASSWORD")
    if not user or not pwd: raise RuntimeError("Missing WORDPRESS_USERNAME or WORDPRESS_PASSWORD")
    token = base64.b64encode(f"{user}:{pwd}".encode()).decode()
    return {"Authorization": f"Basic {token}"}

def _wp_base_url():
    url = os.getenv("WORDPRESS_URL")
    if not url: raise RuntimeError("Missing WORDPRESS_URL")
    return url.rstrip("/")
...
def _upload_featured_image_to_wp(image_b64: str, filename: str = "header.png") -> tuple[int, str]:
    ...
    up_headers = {**headers, "Content-Disposition": f'attachment; filename="{filename}"', "Content-Type": "image/png"}
    ...
    j = r.json()
    media_id = j.get("id"); source_url = j.get("source_url")
...
def publisher_node(state: dict) -> dict:
    ...
    featured_id = None; featured_src = None
    ...
    payload = {"title": title, "content": content_body, "status": "publish"}
    ...
    try:
        ...
        if r.status_code == 201:
            ...
            return {"status": "wp_error", "messages": [HumanMessage(content=f"WP error {r.status_code}")]}
    except Exception as e:
        print(f"❌ Failed to connect to WordPress: {e}", flush=True)
        return {"status": "error", "messages": [HumanMessage(content="WP connection failed")]}
