...
# Podesi mapiranje category_hint -> WP slug ovde:
# Primer: "AI" -> "ai", "Tech" -> "technology"
CATEGORY_SLUG_MAP = {
    "AI": "ai",
    "Tech": "technology",
    "Science": "science",
...
# Ako slug ne postoji u WP ili hint nepoznat, koristi fallback ID iz okruženja:
# WP_DEFAULT_CATEGORY_ID=123
def _wp_default_cat_id() -> Optional[int]:
    v = os.getenv("WP_DEFAULT_CATEGORY_ID", "").strip()
...
def _wp_get_category_id_by_slug(slug: str) -> Optional[int]:
    ...
    # Traži po slug-u; per_page 100 da pokrije više kategorija
...
def _resolve_category_ids(state: dict) -> List[int]:
    # 1) pokušaj iz state["category"] ili original_post["category_hint"]
    hint = (state.get("category") or
            state.get("original_post", {}).get("category_hint") or "").strip()
    ids: List[int] = []

    # 2) mapiraj hint -> slug i potraži u WP
    if hint:
    ...
    # 3) fallback: WP_DEFAULT_CATEGORY_ID (ako postoji)
    if not ids:
...
def _strip_image_prompt_marker(markdown_text: str) -> str:
    # Ukloni linije tipa: [IMAGE_PROMPT]: ...
    return re.sub(r"^\[IMAGE_PROMPT\]:.*?$", "", markdown_text, flags=re.M).strip()

def _md_to_html(md_text: str) -> str:
    if md is None:
        # Minimalni fallback (ako paket nije instaliran) — ali OBAVEZNO instaliraj 'markdown'
        # jer bez ovoga ćeš opet videti #, ** itd.
...
def _insert_inline_figures(html: str, urls: List[str]) -> str:
    """
    Ubaci <figure><img/></figure> posle prvog <h2>; ako nema, na kraj.
    """
...
def publisher_node(state: dict) -> dict:
    ...
    Zahteva:
      ...
      - opciono: state["image_prompt"], state["original_post"]["category_hint"]

    Obavezno: featured (1536x1024) + 2 inline (1024x1024); dodela kategorije.
    """
    raw_md = (state.get("final_article") or "").strip()
    if not raw_md:
        return {"status": "error", "messages": [HumanMessage(content="No final_article to publish")]}

    # Ukloni marker linije tipa [IMAGE_PROMPT]: ...
    raw_md = _strip_image_prompt_marker(raw_md)

    # Title iz prve linije Markdown-a
    ...
    try:
        # 1) Generiši i upload-uj FEATURED (hero)
        ...
        if not cat_ids:
            print("⚠️ No category resolved; WP will use its default (risk of Uncategorized). Consider setting WP_DEFAULT_CATEGORY_ID.", flush=True)
